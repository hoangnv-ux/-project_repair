...
<body>
    <div class="container">
        <h1>{{ __('email_template.email_subject', ['project' => config('app.name')]) }}</h1>

        <p>{{ __('email_template.greeting', ['project' => config('app.name')]) }}</p>

        <p>{{ __('email_template.instruction') }}</p>

        <p style="text-align: center;">
            <a href="{{ $url }}" class="button">{{ __('email_template.button_text') }}</a>
        </p>

        <p>
            {{ __('email_template.expiration_notice', ['hours' => $expiration_time]) }}
        </p>
